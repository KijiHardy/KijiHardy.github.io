...
function createRecommendation() {
   if (acresBox.value <= 5000) { // 5000 acres or less, no crop test needed
      if (monthsBox.value >= 10 && monthsBox.value < 13) { // 10+ months of farming per year
      ...
      if (monthsBox.value <= 9 && monthsBox.value > 0) { // 9 or fewer months per year, no crop test needed
   ...
   if (monthsBox.value > 12){
         messageElement.innerHTML = "Enter a month value from 1-12"
      }
      else if(monthsBox.value < 1){
         messageElement.innerHTML = "Enter a month value from 1-12"
      }
